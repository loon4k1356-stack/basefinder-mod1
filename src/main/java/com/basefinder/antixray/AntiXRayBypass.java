...
public class AntiXRayBypass {
    ...
    private boolean isSuspiciousPosition(World world, BlockPos pos, String blockId) {
        ...
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dz = -1; dz <= 1; dz++) {
                    ...
                    if (COMMON_COVERS.contains(neighborId)) {
                        coverNeighbors++;
                    }
                }
            }
        ...
        return coverNeighbors == totalNeighbors;
    }

    private boolean isHiddenByAntiXRay(World world, BlockPos pos, String blockId) {
        if (COMMON_COVERS.contains(blockId)) {
            boolean hasAdjacentAir = false;
            for (int dx = -1; dx <= 1 && !hasAdjacentAir; dx++) {
                for (int dy = -1; dy <= 1 && !hasAdjacentAir; dy++) {
                    for (int dz = -1; dz <= 1 && !hasAdjacentAir; dz++) {
                        if (dx == 0 && dy == 0 && dz == 0) continue;
                        if (world.getBlockState(pos.add(dx, dy, dz)).isAir()) {
                            hasAdjacentAir = true;
                        }
                    }
                }
            }
            if (hasAdjacentAir && !confirmedBlocks.containsKey(pos)) {
                return false;
            }
        }
        return false;
    ...
    public List<ChunkPos> randomizeChunkOrder(List<ChunkPos> chunks) {
        List<ChunkPos> shuffled = new ArrayList<>(chunks);
        Collections.shuffle(shuffled, random);
        for (int i = 0; i < shuffled.size() / 4; i++) {
            int a = random.nextInt(shuffled.size());
            int b = random.nextInt(shuffled.size());
            ChunkPos temp = shuffled.get(a);
            shuffled.set(a, shuffled.get(b));
            shuffled.set(b, temp);
        }
        return shuffled;
    }
    ...
    public boolean shouldScanNow() {
        ...
        if (currentTime - burstStartTime > 1000) {
            burstStartTime = currentTime;
            burstCount = 0;
        }
        if (burstCount >= burstLimit) {
            if (currentTime - burstStartTime < burstCooldownMs) return false;
            burstStartTime = currentTime;
            burstCount = 0;
        }
        if (currentTime - lastScanTime < minDelayMs) return false;
        int randomDelay = random.nextInt(maxDelayMs - minDelayMs);
        if (currentTime - lastScanTime < minDelayMs + randomDelay / 2) return false;
    ...
    public int getValidatedChunkCount() {
        return validatedChunks.size();
    }
